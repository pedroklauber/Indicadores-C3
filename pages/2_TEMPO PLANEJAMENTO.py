...
if os.path.exists(ARQUIVO):
    ...
    with col1:
        ...
        ax.fill_between(semanas, acima_meta, color="red", alpha=0.4, label="Acima da Meta")
        ...
        ax.axhline(y=meta, color='white', linestyle='--', linewidth=1, label=f"Meta = {meta:.2f} dias")

        ax.set_facecolor('#0e1117')
        ax.set_ylabel("Tempo (dias)", color='white', fontsize=10)
    ...
    with col2:
        ...
        <div style="background-color:#1e1e1e;padding:6px 10px;border-radius:10px;text-align:center">
            ...
            <p style="color:gray;font-size:13px;margin:2px 0">Tempo de Planejamento</p>
            <p style="color:{cor};font-size:16px;margin:4px 0">{emoji} {texto}</p>
            <p style="color:white;font-size:11px;margin:2px 0">Meta: {meta:.2f} dias</p>
        </div>
        """, unsafe_allow_html=True)
